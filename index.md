...
56. Internal结构指的是先给定应该范畴C，然后利用范畴C的构件（对象，态射）为材料构造出来的结构，是范畴C的某种“子结构”。如内蕴群，内蕴环，内蕴函子等等，其构件都源自于给定的范畴C。而外蕴定义则不依赖于给定的范畴C，而是使用集合论的语言给出定义，内蕴定义完全抛弃集合论，在某给定范畴内部去定义相关结构（即取给定范畴的某种子结构）。背景范畴C的结构越丰富，它就有越多的结构帮助我们去构建各种内蕴结构。
