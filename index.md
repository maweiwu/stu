...
51. 函子F性质好坏与否取决于它对范畴的各种“局部结构”的保持性，如：函子是满的，忠实的（对范畴的态射结构的保持性）；函子是本质满的（对范畴的对象这个局部结构的保持性）；函子是正合的（对极限锥这个局部结构的保持性）；函子是三角的（对范畴的三角结构的保持性）等等。

52. 范畴的好坏取决于它所具有的各种结构（各种极限和余极限及其他特殊运算和结构，如有限极限，小极限，滤过余极限，张量积，正合结构，三角结构等等）以及这些结构之间的相容性（如极限与某些余极限的交换性等），这好比代数结构（如模）及它的各种运算的相容性（如数乘运算与加法运算的分配率等等）。  
