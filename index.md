...
253. 谱序列 E_r^{p,q} 最重要的是【由 {p,q,r} 三元数组出发写出 E_r^{p,q} 的前后两项】(这是最重要的技能！)：(1) p +/- r 【第一项为左/右平移】; (2) (n +/- 1) - (1) 【相邻对角线减去第一项】。写出前后两项后判断微分 d_r 【是否为零映射和非零映射(同构或非同构)】。由 d_r 写出下一页 E_{r+1}^{p,q} 并判断是否稳定。 【由 {p,q,r} 产生了 d_r 从而联结了 E_r^{p,q} 和 H^n，使得它们得以比较。谱序列定理中只给出 E_2^{p,q} 的信息，没有给出微分 d_r，但我们可以利用 {p,q,r} 判断它是否为零映射、同构或非同构，即使是非同构也很有意义，因为它至少说明了 d_r 的存在性，即 d_r 前后项是可比较的。】由 {p,q,r} 可以直接判断 E_r^{p,q} 什么时候稳定：r > max{p,q+1}。
